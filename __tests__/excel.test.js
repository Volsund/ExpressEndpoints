...
it("/excel-sum endpoint working with html response", async (done) => {
  const response = await request.post("/excel-sum").set("Accept", "text/html");
...
it("/excel-sum endpoint working with default/plain text response", async (done) => {
  const response = await request.post("/excel-sum").set("Accept", "text/plain");
...
it("/excel-sum GET route working", async (done) => {
  ...
  expect(response.text).toBe(
    `GET route on Excel-Sum. You are probably more interested in POST route...`
  );
